...
export const posts: Post[] = [
  {
    slug: "claude-code-ile-web-sitesi",
    title: "Aynı AI Sitelerinden Kurtulmak: Sır Araçta Değil, Sırada",
    category: "AI & Yazılım",
    excerpt:
      "Bütün vibe coding araçları aynı arayüzü üretir: aynı mor degrade, aynı font, aynı kartlar. Farkı yaratan araç değil, ona verdiğin sıra. Bu sitenin yeniden inşa hikâyesi ve dürüst bir eksik listesi.",
    date: "2026-06-16",
    readTime: "9 dk",
    cover: "linear-gradient(135deg,#219295,#0d1117)",
    tags: ["Claude Code", "vibe coding", "web tasarım", "DESIGN.md", "AI"],
    sources: [
      { title: "Claude Code ile Web Siteni Nasıl Kurarsın?", author: "Defne İncekara", type: "makale", url: "https://www.defneincekara.com", note: "Defne'nin AI Köşesi bülteni — bu yazıdaki disiplinli sıra fikrine ilham oldu" },
      { title: "Claude Code", author: "Anthropic", type: "veri", url: "https://claude.com/product/claude-code", note: "Komut satırından çalışan AI kod ve proje yardımcısı" }
    ],
    content: `
<p>İnternette bir tuhaflık var: yapay zekayla kurulan sitelerin çoğu birbirinin aynısı. Aynı mor degrade arka plan, aynı Inter font, aynı yuvarlak köşeli kartlar, aynı "her şeyi açıklayan" açılış başlığı. Lovable, Replit, v0, Claude Code… hangi aracı açarsan aç, "bana bir site yap" dediğinde benzer bir his alıyorsun. Çünkü bu araçların hepsi <strong>varsayılan bir zevkle</strong> geliyor ve sen ona yön vermezsen o varsayılanı olduğu gibi yayına almış oluyorsun.</p>

<p>Ben de bu tuzağa düştüm. Şu an okuduğun siteyi, <strong>mustafayilmaz.art</strong>'ı, ilk denememde tek hamlede kurdum: "bana bir kişisel site yap" dedim ve bitmesini bekledim. Çıkan şey teknik olarak çalışıyordu ama on binlerce AI sitesinden bir farkı yoktu. Sonra söküp aynı işi baştan, bu sefer disiplinle yaptım. Aradaki fark araçta değildi.</p>

<h2>Sır araçta değil, sırada</h2>

<p>Farkı yaratan şey şuydu: <strong>tek bir dev prompt yerine, baştan sona ilerleyen bir sıra.</strong> Önce gerçekler, sonra metin, sonra tasarım, en son kod. Bu sırayı bozmadığın sürece hangi aracı kullandığın çok da önemli değil. Ben SvelteKit ve Firebase Hosting kullandım, biri Lovable kullanır, biri saf Claude Code — sıra aynıysa sonuç aynı yere çıkar: gerçekten sana ait bir site.</p>

<p>Bu sıranın kıvılcımını, Defne İncekara'nın "Defne'nin AI Köşesi" bültenindeki dokuz adımlı anlatımından aldım (kaynaklara ekledim). Ama aşağıdaki her şey kendi sitemde, kendi araçlarımla yaşadığım haliyle.</p>

<h2>Önce gerçekler: tek bir doğruluk kaynağı</h2>

<p>İlk adım kod değil, <em>gerçekler</em>. Sitenin üstüne kurulacağı bilgileri tek bir dosyada topladım: kim olduğum, ne yaptığım, ses kurallarım. Hiçbir şey uydurulmadı — uydurma referans, uydurma istatistik, uydurma rozet yok. Bu benim için pazarlık konusu değil; "üfürme yok" zaten markamın temel kuralı.</p>

<p>Önemli bir incelik: AI seni geçmiş konuşmalardan tanıyor olabilir ama sitenin <em>senin hakkında</em> olduğunu varsaymamalı. Sitenin konusu neyse odak odur. Emin değilse sormalı, tahmin etmemeli.</p>

<h2>Sonra metin: kod yazılmadan her kelimeyi onayla</h2>

<p>Sayfa metnini ben yazmadım; soruları cevapladım, taslağı AI yazdı. Önce sayfa yapısını önerdi, onayladım. Sonra her sayfa için sadece eksik kalanları sordu — bildiğini tekrar sormadı. Çıkan metni, <strong>tek satır kod yazılmadan</strong> baştan sona okudum.</p>

<p>Bu adımı atlama. Metni baştan netleştirmek, sonradan kurulu sayfaları tek tek düzeltmekten çok daha ucuz. Arama terimleri başlıklara ve açılış cümlelerine doğal biçimde yerleşmeli — zorlama, anahtar kelime yığma değil.</p>

<h2>Sonra tasarım: markanı sevdiğin sitelerle harmanla</h2>

<p>Tasarımın kural kitabı tek bir <strong>DESIGN.md</strong> dosyasıdır. Sevdiğin sitelerin tasarım dilini topla, kendi marka kitini ekle ve AI'a "bunları birleştir" de. Kural net: <em>renkte ve fontta senin markan kazanır, yerleşim ve histe referanslar kazanır.</em></p>

<p>Bu sitede o kural kitabı şöyle çıktı: başlıklarda <strong>Playfair Display</strong> serif, gövdede <strong>DM Sans</strong>, etiketlerde <strong>JetBrains Mono</strong>; teal (#219295) ve sıcak sarı vurgu, krem zemin. Mor degrade yok — vurgu rengim teal'den sarıya geçen bir gradient. Renkleri tek tek tekrarlamak yerine üç katmanlı bir <em>token</em> sistemine bağladım: bir tonu değiştirmek için tek satır yetiyor, bütün site takip ediyor.</p>

<h2>En son kod: kur, sonra denetle</h2>

<p>Sıra geldiğinde DESIGN.md stil token'larına, metin sayfalara dönüşür. Ama asıl mesele kurduktan <em>sonra</em> ne yaptığın. Ben her değişiklikten sonra otomatik tip denetimi ve derleme çalıştırıyorum; sonra bağımsız uzman gözlerle denetletiyorum: SEO, erişilebilirlik (WCAG), performans, çapraz tarayıcı.</p>

<p>Bu denetimler gerçek eksikler buldu: 50'den fazla sayfa Google'a yanlış bildiriliyormuş; iOS Safari'de buzlu-cam efekti bozukmuş; küçük dokunma hedefleri parmakla zor seçiliyormuş. Hepsi tek tek kapandı. "Çalışıyor" ile "iyi" arasındaki fark bu denetim katmanında.</p>

<h2>Bileşenleri "donör" olarak kullan</h2>

<p>Hazır bileşen kütüphanelerinden (21st.dev, Aceternity gibi) bir prompt yapıştırdığında, o bileşeni <strong>yapısal bir donör</strong> olarak ele al: iskeletini al, ama demo metnini kendi metninle, hardcoded renklerini DESIGN.md token'larınla değiştir, stok görsel talimatlarını yok say. Bileşen iskeleti verir, DESIGN.md deriyi, metnin kelimeleri. Böylece her bileşen senin sitene benzer, kendi kıyafetiyle gelmez.</p>

<h2>Zevk denetimi: "eleştirmen ne derdi?"</h2>

<p>Yayına almadan önce tek bir soru çok şey değiştiriyor: <em>"Bir tasarım eleştirmeni bu sayfaya ne derdi?"</em> Bu soru, sitedeki "AI yapımı" hissini yaratan klişeleri yüzeye çıkarır: mor degradeler, üç sütunlu ikon grid'leri, her şeyi ortalamak, yapısal ikon yerine emoji kullanmak.</p>

<p>Bu yazıyı yazarken tam da bunu yaptım: sitedeki bütün emoji ikonları elle çizilmiş SVG ikonlara çevirdim. Küçük bir detay gibi ama "amatör" ile "kasıtlı" arasındaki çizgi çoğu zaman bu detaylarda.</p>

<h2>Bizde eksik kalanlar (dürüst liste)</h2>

<p>Bu siteyi mükemmel diye sunmak markama aykırı olur. Bu yöntemi tam uygulamadığımız, hâlâ açık olan yerler var:</p>

<ul>
  <li><strong>Açılış videosu yok.</strong> İdeal akışta hero, markanın paletinden üretilmiş sakin bir video olur. Bizimki şimdilik statik bir kompozisyon — bilinçli bir tercih, ama bir gün kendi paletimizden bir hero videosu eklemeye değer.</li>
  <li><strong>Görsel/ekran görüntüsü denetim döngüsü yok.</strong> Kodu uzman gözlerle denetledik ama "her sayfanın ekran görüntüsünü al, göz kararıyla incele" adımını henüz otomatikleştirmedik. Kod doğru olabilir, göze çarpan bir boşluk yine de kaçabilir.</li>
  <li><strong>Metin ve gerçekler ayrı dosyada değil.</strong> İçerik sayfalara ve veri dosyalarına dağılmış durumda; tek bir merkezde toplamadık. Şu an iyi çalışıyor ama büyüdükçe tek kaynak daha sağlam olur.</li>
  <li><strong>Fontlar hâlâ dış sunucudan.</strong> Google Fonts üçüncü taraf bir kaynak; KVKK hassasiyeti ve hız için bir gün kendi sunucumuzdan servis etmeye değer.</li>
</ul>

<p>Bunları gizlemek yerine yazıyorum, çünkü dürüst eksik listesi, parlak vaatten daha çok işe yarar.</p>

<h2>Kendi alan adında dene</h2>

<p>Web siteleri eskiden zordu. Şimdi gereken tek şey: sahip olduğun bir alan adı, bir gerçekler dosyası ve bu sırayı elinde tutmak. İşin sırrı adımları ezberlemekte değil, <strong>sırayı sen belirlemekte</strong>. Doğru sırayı sen koymazsan, dünyanın en iyi aracıyla bile herkesle aynı siteyi elde edersin.</p>

<p>Sonunda ortaya çıkan site gerçekten sana ait olur: senin kelimelerin, senin renklerin, senin kanıtların. Stok görsel yok, stok metin yok. Bir tane kurarsan, linkini bana yolla — birbirimizden ilham alalım.</p>
`,
  },
  {
