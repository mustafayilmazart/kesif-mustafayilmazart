# Atıflar & Yararlanılan Kaynaklar

## Çekirdek Bağımlılıklar

| Paket | Lisans | Kullanım |
|---|---|---|
| [Next.js](https://nextjs.org) | MIT | React framework |
| [React](https://react.dev) | MIT | UI library |
| [Tailwind CSS](https://tailwindcss.com) | MIT | Stil framework |
| [TypeScript](https://www.typescriptlang.org) | Apache 2.0 | Type system |

## Yazı Tipleri (Google Fonts)

Tümü [SIL Open Font License (OFL)](https://scripts.sil.org/OFL) altındadır:
- [Playfair Display](https://fonts.google.com/specimen/Playfair+Display) — Claus Eggers Sørensen
- [DM Sans](https://fonts.google.com/specimen/DM+Sans) — Colophon Foundry, Jonny Pinhorn
- [JetBrains Mono](https://www.jetbrains.com/lp/mono/) — JetBrains s.r.o.

## Hosting & Altyapı

- [Firebase Hosting](https://firebase.google.com/docs/hosting) — © Google LLC
- [Cloudflare DNS / CDN](https://www.cloudflare.com) — © Cloudflare, Inc.

## Tasarım & UX İlhamı

Site tasarımı şu projelerden ilham aldı (kod/asset kopyalanmamış, yalnızca yaklaşım):

- [**alirezarezvani.com**](https://alirezarezvani.com) — Hero + skill grid + timeline yapısı
- [**fs.blog**](https://fs.blog/reading/) — Blog liste sayfası kompozisyonu
- [**jamesclear.com**](https://jamesclear.com/creative-thinking) — Tekil makale tipografisi
- [**verywellmind.com**](https://www.verywellmind.com) — Kategori + okuma süresi metadata yapısı

## İçerik Hakları

- **Profil fotoğrafları** (`public/profile.jpg`, `profile-large.jpg`) — © Mustafa Yılmaz. Eğer üçüncü taraf bir fotoğrafçı tarafından çekildiyse, FSEK m.86 kapsamında eser sahibi fotoğrafçıdır ve kullanım için yazılı izin alınmıştır. Fotoğraflar kişisel/portfolyo kullanım kapsamında bu sitede yer almaktadır.
- **Blog yazıları** — © Mustafa Yılmaz, **All Rights Reserved**. (LICENSE dosyasındaki Dual Licensing notu geçerlidir; kişisel içerik MIT kapsamı dışındadır.) Yeniden yayınlamak için yazılı izin gerekir; alıntı yapmak isteyenler [bilgi@mustafayilmaz.art](mailto:bilgi@mustafayilmaz.art) adresine yazabilir.
- **Udemy yorumları** (Referanslar bölümü) — Udemy platformunda öğrencilerin **kamuya açık** yayınladığı 5★ yorumları. **KVKK uyumu için soyadlar kısaltılmıştır** (örn. "Fazilet O." biçiminde). Yorum metni birebir alıntıdır; yorum sahibi açık rıza vermediği takdirde tam ad-soyad kullanımı KVKK Md. 5 kapsamında risk yaratabileceğinden, yalnızca **ad + soyad inisiyali** gösterilmiştir. Bir yorum sahibi yorumunun kaldırılmasını isterse [bilgi@mustafayilmaz.art](mailto:bilgi@mustafayilmaz.art) adresine yazabilir; talep 7 gün içinde işleme alınır.

## Vector Icons

Tüm SVG ikonlar [Feather Icons](https://feathericons.com) (MIT) ve [Lucide](https://lucide.dev) (ISC) kütüphanelerinden / yaklaşımından alınmıştır. Bazıları sıfırdan çizilmiştir.

## Markalar

Sitede anılan tüm 3. taraf markalar (Udemy, Medium, Substack, Suno, Claude, vb.) ilgili sahiplerinin tescilli markalarıdır. Yalnızca tanımlama amaçlı (nominative fair use) kullanılmıştır.
